...
class RNDModule:
    ...
    def get_intrinsic_reward_normalized(self, state, intrinsic_rewards_batch=None):
        """
        计算归一化后的内在奖励（用于策略更新）
        
        Args:
            state: 当前状态
            intrinsic_rewards_batch: 可选，用于批量更新归一化统计量
        ...
        # 更新归一化统计量
        if intrinsic_rewards_batch is not None:
            self.intrinsic_rms.update(intrinsic_rewards_batch)
        else:
    ...
    def get_scaled_intrinsic_reward(self, state, current_episode, total_episodes):
        ...
        Returns:
            scaled_intrinsic: 最终用于策略更新的内在奖励值
            raw_intrinsic: 原始内在奖励（用于日志记录）
            decay_coef: 当前衰减系数（用于日志记录）
        """
        # 计算归一化后的内在奖励
        normalized_intrinsic = self.get_intrinsic_reward_normalized(state)
...
class MetricsLogger:
    ...
    def __init__(self, output_prefix="llama_ppo_rnd"):
        ...
        self.output_dir = f"/home/wuyang/results/llama_ppo_rnd_results/0415"
...
class Trajectory:
    ...
    value: float
    state_str: str
    intrinsic_reward: float = 0.0  # 内在奖励（可选）
...
class PPOBuffer:
    ...
    关键设计原则：
    1. 内在奖励独立归一化，不影响外部奖励
    2. Value函数只学习外部奖励（保守策略，避免Value函数被内在奖励干扰）
    3. Advantage可以包含内在奖励的贡献，影响Policy更新
    """
    ...
    def compute_gae_and_normalize(self, last_value, use_intrinsic=True, intrinsic_coef=0.01):
        ...
        关键设计：
        1. 外部奖励归一化：使用batch统计量
        2. 内在奖励：已在RNDModule中归一化
        3. 组合奖励 = 外部奖励 + intrinsic_coef * 内在奖励（如果启用）
        4. Value函数只学习外部奖励（returns只用外部奖励计算）
        5. Advantage可以使用组合奖励（引导探索）
        
        Args:
            last_value: 最后状态的估值
            use_intrinsic: 是否在Advantage计算中使用内在奖励
            intrinsic_coef: 内在奖励系数
        
        Returns:
            advantages: 用于Policy更新的优势函数
            returns: 用于Value学习的回报（只用外部奖励）
        """
        ...
        # 3. GAE计算（关键：区分用于Policy和用于Value的奖励）
        ...
        if use_intrinsic and intrinsic_coef > 0 and int_rewards.sum() != 0:
            ...
            for t in reversed(range(n)):
                if t == n - 1:
                    next_value = 0.0  # 内在奖励的future value视为0（已归一化）
                    next_non_terminal = 1.0 - dones_t[t]
                else:
                    next_value = 0.0  # 简化：内在奖励不累积
                    next_non_terminal = 1.0 - dones_t[t]
                
                # 内在奖励的delta（简化：不使用value bootstrapping）
                delta_int = int_rewards_t[t] + self.gamma * next_value * next_non_terminal
                gae_int = delta_int + self.gamma * self.gae_lambda * next_non_terminal * gae_int
                advantages_int[t] = gae_int
            
            # 组合Advantages：外部 + 系数 * 内在
...
class PPOAgent:
    def __init__(self, llama_model, tokenizer, config, actual_state_size=None):
        ...
        # 4. 混合精度
        self.scaler = torch.cuda.amp.GradScaler(enabled=not config.use_bf16) # BF16通常不需要Scaler
        ...
        # 7. RND模块 - 使用实际状态维度
        # 修复：如果提供了actual_state_size则使用它，否则使用config.state_size
    ...
    def update(self, last_value, current_episode=0, total_episodes=None):
        ...
        Args:
            last_value: 最后状态的估值
            current_episode: 当前episode（用于内在奖励衰减）
            total_episodes: 总episode数
        """
        self.embedder.train()
...
def train_ppo():
    print("=" * 70)
    print("PPO + LLaMA Training Started (Enhanced Version)")
    print("=" * 70)
    ...
    env_args = {
        ...
        'mode': 'all', 'seed': 123456, 'random_weather': False,'fileX_template_path':'/home/wuyang/test/val_proj/SIAZ9501.MZX'
    }
    env = gym.make('gym_dssat_pdi:GymDssatPdi-v0', **env_args)
    
    # 获取实际状态维度（修复RND输入维度不匹配问题）
    initial_state = dict2array(env.reset())
    actual_state_size = len(initial_state)
    print(f"   Actual state size: {actual_state_size} (config.state_size: {config.state_size})")
    ...
    for episode in pbar:
        ...
        for step in range(config.max_steps_per_episode):
            ...
            if config.use_rnd_intrinsic:
                # 获取归一化和衰减后的内在奖励
                scaled_intrinsic, norm_intrinsic, decay_coef = agent.rnd.get_scaled_intrinsic_reward(
                    state, episode, config.n_episodes
                )
                # 存入Buffer，包含内在奖励
                agent.buffer.add(state, action, reward, next_state, done, log_prob, value, 
                               array2str(state), intrinsic_reward=norm_intrinsic)
            else:
                # 不使用内在奖励时，intrinsic_reward为0
                agent.buffer.add(state, action, reward, next_state, done, log_prob, value, 
