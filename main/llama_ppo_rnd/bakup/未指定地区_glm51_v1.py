#!/usr/bin/env python3
"""
PPO + Chinese-LLaMA-2 + gym-dssat 深度优化修复版
针对 A100 40G 优化

修复核心:
1. 梯度流修复 - 训练时强制重新计算嵌入，确保LLaMA和投影层参与反向传播
2. 缓存一致性 - 训练阶段禁用缓存，避免旧参数嵌入干扰策略更新
3. 奖励归一化增强 - 在GAE计算前统一进行PopArt归一化，稳定Value Function学习
4. 梯度累积修正 - 修正zero_grad位置，防止梯度累积错误
5. 学习率调整 - 提高Actor-Critic头学习率，加速收敛
6. BF16支持 - 利用A100的BF16加速并提升数值稳定性

新增功能:
7. 农学指标输出 - 最终产量、灌溉/施肥量、WUE、NUE
8. AI指标输出 - 平均回报、样本效率、收敛速度、探索覆盖率
9. 结果保存 - PNG/PDF/Excel格式
10. 时间进度条 - 训练过程可视化

[RND-INTEGRATION] RND内在奖励参与策略更新 (保守集成版):
11. RND内在奖励通过 β·r_intrinsic 叠加到外征奖励上，参与GAE和策略梯度计算
12. 使用Running Mean/Std (Welford在线算法) 流式归一化内在奖励，裁剪至[-5,5]
13. 内在奖励在Buffer外征奖励批量归一化之后叠加，确保外征归一化逻辑不变
14. β=intrinsic_coef=0.01 (默认)，极保守系数，确保不破坏原有收敛
15. β=0时行为与原代码完全一致，可随时回退做消融实验
16. 验证阶段仅使用外征奖励，不叠加内在奖励
17. RND预测器保持独立优化器，与策略优化解耦
"""
...
class PPOConfig:
    ...
    # === [RND-INTEGRATION] RND内在奖励参数 ===
    intrinsic_coef: float = 0.01        # 内在奖励系数(β)，0=完全禁用(退化为原代码)，推荐0.01
                                        # 消融实验参考: {0, 0.001, 0.01, 0.02}
    intrinsic_clip: float = 5.0         # 归一化内在奖励裁剪范围[-clip, +clip]
    
    # === 指标计算参数 ===
    expert_performance_threshold: float = 500.0  # 专家水平阈值（用于样本效率）
    convergence_window: int = 50                 # 收敛判定窗口大小
    convergence_threshold: float = 0.05          # 收敛判定阈值（方差变化率）
    rnd_threshold: float = 0.1                   # RND内在奖励衰减阈值
...
# ============================================================================
#                  [RND-INTEGRATION] Running Mean/Std 归一化器
#          使用Welford在线算法，流式更新统计量，用于归一化RND内在奖励
# ============================================================================

class RunningMeanStd:
    """
    Welford在线算法计算运行均值和方差。
    用于在rollout过程中流式归一化RND内在奖励，避免batch归一化的噪声问题。
    
    参考:
    - Burda et al., "Exploration by Random Network Distillation" (2018), Section 3.2
    - CleanRL RND实现中的归一化策略
    - Welford, B.P. (1962). "Note on a method for calculating corrected sums of squares and products"
    
    设计考量:
    - 在线更新: 每步仅需O(1)计算，无需存储历史数据
    - 数值稳定: 使用Welford算法避免大数相减的精度损失
    - 裁剪保护: 归一化后裁剪到[-clip, +clip]，防止早期统计量不准时的异常值
    - 初始方差为1.0: 在样本量不足时，归一化效果近似恒等变换，不会产生异常激励
    """
    
    def __init__(self, eps=1e-8, clip=5.0):
        self.mean = 0.0
        self.var = 1.0       # 初始方差为1，使得早期归一化近似恒等变换
        self.count = 0
        self.eps = eps
        self.clip = clip
    ...
    def normalize(self, x):
        ...
        # 归一化
        std = math.sqrt(self.var / self.count) + self.eps
    ...
    def get_stats(self):
        """返回当前统计量，用于日志记录"""
        std = math.sqrt(self.var / max(self.count, 1)) + self.eps
        return {'mean': self.mean, 'std': std, 'count': self.count}
...
#                    用于探索覆盖率计算 + [RND-INTEGRATION] 内在奖励
# ============================================================================
...
class RNDModule:
    """RND模块用于计算探索覆盖率 + [RND-INTEGRATION] 提供内在奖励"""
    def __init__(self, state_size, hidden_size=128, lr=1e-4):
        ...
        # [RND-INTEGRATION] RND预测器保持独立优化器，与策略优化完全解耦
        # 原因: RND的优化目标(最小化预测误差)与策略的优化目标(最大化回报)完全不同
        # 共享优化器会导致梯度交叉污染，使内在信号不可靠
    ...
    def get_intrinsic_reward(self, state):
        ...
        [RND-INTEGRATION] 优化: 添加torch.no_grad()包裹预测器前向传播
        原代码中预测器前向传播会创建计算图但随即被.item()丢弃，浪费显存和计算。
        用no_grad()避免不必要的计算图构建，因为此处仅需标量值用于归一化。
        """
    ...
    def get_exploration_coverage(self, estimated_total_states=1000):
        """计算探索覆盖率"""
        # 基于已探索状态数和估计的总状态空间
        explored_unique = len([s for s, c in self.state_visit_counts.items() 
                               if c > 0])
...
class MetricsLogger:
    ...
    def __init__(self, output_prefix="llama_ppo_rnd"):
        ...
        self.output_dir = f"/home/wuyang/results/llama_ppo_rnd_results/0414"
        ...
        # [RND-INTEGRATION] 内在奖励追踪指标
        self.episode_intrinsic_rewards = []     # 每轮平均内在奖励(归一化后)
        self.intrinsic_contribution_ratios = [] # 内在奖励贡献比例
    ...
    def record_episode(self, episode, yield_val, irrigation, fertilizer, 
                       ...
                       intrinsic_reward_mean=0.0, intrinsic_contribution=0.0):
        """记录单轮训练数据"""
        ...
        # [RND-INTEGRATION] 记录内在奖励指标
        self.episode_intrinsic_rewards.append(intrinsic_reward_mean)
        self.intrinsic_contribution_ratios.append(intrinsic_contribution)
    ...
    def get_current_metrics(self, episode):
        ...
        metrics = {
            ...
            # [RND-INTEGRATION] 内在奖励指标
            'intrinsic_reward_mean': self.episode_intrinsic_rewards[-1] if self.episode_intrinsic_rewards else 0,
            'intrinsic_contribution': self.intrinsic_contribution_ratios[-1] if self.intrinsic_contribution_ratios else 0,
            # 训练信息
    ...
    def print_metrics(self, episode, phase="Training"):
        ...
        # [RND-INTEGRATION] 打印内在奖励指标
        print(f"\n  [RND Intrinsic Reward]")
        print(f"    Intrinsic Mean:     {metrics['intrinsic_reward_mean']:.4f}")
        print(f"    Intrinsic Contrib:  {metrics['intrinsic_contribution']:.4f}")
        print(f"    Beta (intrinsic_coef): {config.intrinsic_coef}")
    ...
    def _save_to_excel(self):
        ...
        data = {
            ...
            # [RND-INTEGRATION] 新增内在奖励列
            'Intrinsic_Reward_Mean': self.episode_intrinsic_rewards,
            'Intrinsic_Contribution_Ratio': self.intrinsic_contribution_ratios,
        }
        ...
        df['Intrinsic_Coef_Beta'] = config.intrinsic_coef  # [RND-INTEGRATION] 记录β值
        ...
        with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
            ...
            summary_data = {
                'Metric': [
                    ...
                    'Avg_Intrinsic_Reward_Mean',
                    'Avg_Intrinsic_Contribution_Ratio',
                    'Intrinsic_Coef_Beta'
                ],
                'Value': [
                    ...
                    np.mean(self.episode_intrinsic_rewards) if self.episode_intrinsic_rewards else 0,
                    np.mean(self.intrinsic_contribution_ratios) if self.intrinsic_contribution_ratios else 0,
                    config.intrinsic_coef
                ]
            }
    ...
    def _save_to_pdf(self):
        ...
        elements.append(Paragraph("PPO + LLaMA Training Report (RND-Integrated)", title_style))
        ...
        summary_data = [
            ...
            # [RND-INTEGRATION] 新增内在奖励统计行
            ['Intrinsic Coef (Beta)', f"{config.intrinsic_coef}"],
            ['Avg Intrinsic Reward Mean', f"{np.mean(self.episode_intrinsic_rewards) if self.episode_intrinsic_rewards else 0:.4f}"],
            ['Avg Intrinsic Contribution', f"{np.mean(self.intrinsic_contribution_ratios) if self.intrinsic_contribution_ratios else 0:.4f}"],
        ]
        ...
        if self.metrics_every_10:
            ...
            # [RND-INTEGRATION] 表头增加内在奖励列
            metrics_10_data = [['Episode', 'Yield (kg/ha)', 'WUE', 'NUE', 'Avg Reward', 'Exploration (%)', 'Intrinsic Mean', 'Intrinsic Contrib']]
            for m in self.metrics_every_10:
                metrics_10_data.append([
                    ...
                    f"{m['intrinsic_reward_mean']:.4f}",
                    f"{m['intrinsic_contribution']:.4f}"
                ])
            
            table_10 = Table(metrics_10_data, colWidths=[0.8*inch, 1*inch, 0.8*inch, 0.8*inch, 0.9*inch, 1*inch, 0.9*inch, 0.9*inch])
    ...
    def _save_plots(self):
        ...
        # [RND-INTEGRATION] 创建4x3子图(原3x3基础上增加1行内在奖励图)
        fig, axes = plt.subplots(4, 3, figsize=(15, 16))
        fig.suptitle('PPO + LLaMA Training Metrics (RND-Integrated)', fontsize=16)
        ...
        # [RND-INTEGRATION] 新增第4行: 内在奖励相关图表
        # 10. 内在奖励均值曲线
        ax = axes[3, 0]
        if self.episode_intrinsic_rewards:
            ax.plot(episodes, self.episode_intrinsic_rewards, 'teal', alpha=0.7)
        ax.set_xlabel('Episode')
        ax.set_ylabel('Intrinsic Reward Mean')
        ax.set_title(f'RND Intrinsic Reward (Beta={config.intrinsic_coef})')
        ax.grid(True, alpha=0.3)
        
        # 11. 内在奖励贡献比例
        ax = axes[3, 1]
        if self.intrinsic_contribution_ratios:
            ax.plot(episodes, self.intrinsic_contribution_ratios, 'coral', alpha=0.7)
        ax.set_xlabel('Episode')
        ax.set_ylabel('Intrinsic Contribution')
        ax.set_title('Intrinsic Reward Contribution Ratio')
        ax.grid(True, alpha=0.3)
        
        # 12. 外征回报 vs 外征+内在回报 对比
        ax = axes[3, 2]
        ax.plot(episodes, self.episode_rewards, 'b-', alpha=0.5, label='Extrinsic Reward')
        # 计算含内在奖励的总回报(近似)
        if self.episode_intrinsic_rewards:
            combined_rewards = [r + config.intrinsic_coef * ir * steps 
                               for r, ir, steps in zip(self.episode_rewards, 
                                                         self.episode_intrinsic_rewards,
                                                         self.episode_steps)]
            ax.plot(episodes, combined_rewards, 'r-', alpha=0.5, label='Extrinsic+Intrinsic')
        ax.set_xlabel('Episode')
        ax.set_ylabel('Total Reward')
        ax.set_title('Extrinsic vs Combined Reward')
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
        ...
        ax2.set_title(f'Training Reward Curve with Key Milestones (Beta={config.intrinsic_coef})', fontsize=14)
        ax2.legend(loc='best')
...
# ============================================================================
#                       PPO Buffer (修复版 + [RND-INTEGRATION])
# ============================================================================
...
class Trajectory:
    ...
    value: float
    state_str: str
    intrinsic_reward: float = 0.0    # [RND-INTEGRATION] 归一化后的内在奖励
...
class PPOBuffer:
    ...
    def add(self, state, action, reward, next_state, done, log_prob, value, state_str, 
            intrinsic_reward=0.0):  # [RND-INTEGRATION] 新增intrinsic_reward参数
        self.trajectories.append(Trajectory(state, action, reward, next_state, done, 
                                            log_prob, value, state_str, intrinsic_reward))
    
    def clear(self): self.trajectories = []
    def __len__(self): return len(self.trajectories)
    
    def compute_gae_and_normalize(self, last_value, intrinsic_coef=0.0):
        """
        计算GAE并在内部进行奖励归一化。
        
        [RND-INTEGRATION] 关键设计决策:
        内在奖励在"外征奖励批量归一化之后"叠加，而非之前。
        
        原因:
        1. 外征奖励的批量归一化逻辑完全不变，保证原有收敛行为不受影响
        2. 内在奖励已通过RunningMeanStd流式归一化(零均值、近似单位方差)
        3. 叠加后的总奖励 = norm(r_ext) + β * norm(r_int)
           - 方差 ≈ 1 + β² ≈ 1.0001 (β=0.01时)
           - 均值 ≈ 0
           - 统计上与原始归一化外征奖励几乎相同
        4. β=0时，代码路径与原版完全一致(恒等变换)
        5. 避免了"先叠加再归一化"导致β系数被重新缩放、耦合外征/内在统计量的问题
        
        参考: Burda et al., 2018 Section 3.2; CleanRL RND实现
        """
        ...
        # [RND-INTEGRATION] 3. 在外征归一化之后叠加内在奖励
        # 内在奖励已在rollout时通过RunningMeanStd归一化(零均值、近似单位方差)
        # 叠加β*intrinsic后，总奖励统计特性几乎不变(β=0.01时方差增加0.01%)
        if intrinsic_coef > 0:
            intrinsic_rewards = np.array([t.intrinsic_reward for t in self.trajectories])
            intrinsic_t = torch.tensor(intrinsic_rewards, dtype=torch.float32, device=self.device)
            rewards_t = rewards_t + intrinsic_coef * intrinsic_t
...
class PPOAgent:
    def __init__(self, llama_model, tokenizer, config):
        ...
        # [RND-INTEGRATION] 8. 内在奖励流式归一化器
        # 使用Welford在线算法，在rollout时逐步更新统计量
        # 归一化后的内在奖励具有近似零均值和单位方差
        self.intrinsic_normalizer = RunningMeanStd(
            eps=1e-8, 
            clip=config.intrinsic_clip
        )
    ...
    def update(self, last_value):
        ...
        # 1. 计算GAE与归一化奖励
        # [RND-INTEGRATION] 传入intrinsic_coef，在GAE计算中叠加内在奖励
...
def validate(agent, env, config, logger, episode):
    ...
    [RND-INTEGRATION] 验证阶段仅使用外征奖励，不叠加内在奖励。
    原因:
    1. 验证的目的是衡量任务性能(产量、WUE、NUE等农学指标)
    2. 内在奖励会膨胀对"新颖但农学上不佳"状态的评分
    3. 保持与无RND基线的公平对比
    """
    print(f"\n{'='*50}")
    ...
    val_metrics = {
        ...
        # [RND-INTEGRATION] 验证中也记录内在奖励统计(仅用于诊断，不参与策略)
        'intrinsic_reward_mean': logger.episode_intrinsic_rewards[-1] if logger.episode_intrinsic_rewards else 0,
        'intrinsic_contribution': logger.intrinsic_contribution_ratios[-1] if logger.intrinsic_contribution_ratios else 0,
    }
    ...
    print(f"    (Extrinsic-only, no intrinsic reward)")
    print(f"{'='*50}\n")
...
def train_ppo():
    print("=" * 70)
    print("PPO + LLaMA Training Started (RND-Integrated Version)")
    print(f"   Intrinsic Reward Coefficient (Beta): {config.intrinsic_coef}")
    print(f"   (Beta=0 disables intrinsic reward, identical to original code)")
    print("=" * 70)
    ...
    # [RND-INTEGRATION] 内在奖励统计追踪
    episode_intrinsic_rewards_raw = []   # 每步原始内在奖励
    episode_intrinsic_rewards_norm = []  # 每步归一化后内在奖励
    
    print(f"\nStarting training ({config.n_episodes} episodes)")
    print(f"Results will be saved to: {logger.output_dir}")
    print(f"RND Intrinsic Coef (Beta): {config.intrinsic_coef}")
    print(f"RND Intrinsic Clip: {config.intrinsic_clip}")
    ...
    for episode in pbar:
        ...
        # [RND-INTEGRATION] 每轮重置内在奖励追踪
        ep_intrinsic_raw_sum = 0.0
        ep_intrinsic_norm_sum = 0.0
        ...
        for step in range(config.max_steps_per_episode):
            ...
            # [RND-INTEGRATION] 计算RND内在奖励 → 归一化 → 参与策略更新
            # 步骤1: 获取原始内在奖励(预测误差)
            intrinsic_reward_raw = agent.rnd.get_intrinsic_reward(state)
            
            # 步骤2: 使用RunningMeanStd流式归一化
            # 这确保内在奖励具有近似零均值和单位方差，与外征奖励尺度匹配
            intrinsic_reward_norm = agent.intrinsic_normalizer.normalize(intrinsic_reward_raw)
            
            # 步骤3: 更新RND预测器(独立优化器，不影响策略梯度)
            agent.rnd.update_predictor(state)
            
            # 步骤4: 更新探索统计
            agent.rnd.update_exploration_stats(state, intrinsic_reward_raw)
            
            # [RND-INTEGRATION] 步骤5: 将归一化后的内在奖励存入Buffer
            # 内在奖励将在compute_gae_and_normalize()中通过 β * intrinsic 叠加到外征奖励上
            # 当β=0时，intrinsic_reward被忽略，行为与原代码完全一致
            agent.buffer.add(state, action, reward, next_state, done, log_prob, value, 
                           array2str(state), intrinsic_reward_norm)
            
            # 追踪内在奖励统计
            ep_intrinsic_raw_sum += intrinsic_reward_raw
            ep_intrinsic_norm_sum += intrinsic_reward_norm
        ...
        # [RND-INTEGRATION] 计算内在奖励统计
        intrinsic_reward_mean = ep_intrinsic_norm_sum / max(steps, 1)
        # 内在奖励贡献比例 = |β * Σr_intrinsic| / |Σr_extrinsic|
        intrinsic_contribution = 0.0
        if abs(episode_reward) > 1e-8:
            intrinsic_contribution = abs(config.intrinsic_coef * ep_intrinsic_norm_sum) / abs(episode_reward)
        
        # 记录指标 (外征奖励保持不变，内在奖励统计单独追踪)
        logger.record_episode(episode, ep_yield, w_amount, n_amount, 
                             episode_reward, steps, exploration_coverage,
                             intrinsic_reward_mean, intrinsic_contribution)
        ...
        pbar.set_postfix({
            ...
            'int_mean': f'{intrinsic_reward_mean:.2f}'
        })
        ...
        if episode % config.update_frequency == 0 and len(agent.buffer) >= config.mini_batch_size:
            ...
            # [RND-INTEGRATION] update内部会调用compute_gae_and_normalize(intrinsic_coef=β)
            # 内在奖励通过 β * r_intrinsic 叠加到归一化后的外征奖励上
            loss = agent.update(last_val)
        ...
        if episode % 10 == 0:
            ...
            # [RND-INTEGRATION] 每10轮打印归一化器统计(诊断用)
            norm_stats = agent.intrinsic_normalizer.get_stats()
            print(f"  [RND Normalizer Stats] Mean={norm_stats['mean']:.4f}, "
                  f"Std={norm_stats['std']:.4f}, Count={norm_stats['count']}")
    ...
    print(f"  Intrinsic Coef (Beta): {config.intrinsic_coef}")
    print(f"{'='*70}")
    
    # 打印最终指标
    logger.print_metrics(config.n_episodes, phase="Final")
    
    # [RND-INTEGRATION] 打印RND归一化器最终统计
    norm_stats = agent.intrinsic_normalizer.get_stats()
    print(f"  [RND Normalizer Final] Mean={norm_stats['mean']:.4f}, "
          f"Std={norm_stats['std']:.4f}, Total Samples={norm_stats['count']}")
