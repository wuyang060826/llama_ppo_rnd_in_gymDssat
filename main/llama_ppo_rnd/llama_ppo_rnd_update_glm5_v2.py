...
class PPOConfig:
    ...
    # 增强开始和完全激活的训练进度
    aug_start_progress: float = 0.3      # 30%进度后开始增强
    aug_full_progress: float = 0.7       # 70%进度时增强完全激活
...
class PolicyEnsemble:
    ...
    def add_checkpoint(self, model_state: dict, score: float):
        if score > self.best_score:
            ...
            self.best_checkpoint = copy.deepcopy(model_state)
        
        self.checkpoints.append(copy.deepcopy(model_state))
...
def train_ppo():
    ...
    for episode in pbar:
        ...
        # 保存最佳
        if episode_reward > best_score and episode_reward > 1300:
            best_score = episode_reward
            agent.save_checkpoint(
                f'/home/wuyang/checkpoints/llama_ppo_rnd_checkpoints/best_0415', 
