...
class MetricsLogger:
    ...
    def __init__(self, output_prefix="llama_ppo_rnd"):
        ...
        self.output_dir = f"/home/wuyang/results/llama_ppo_rnd_results/0411"
...
class PPOBuffer:
    ...
    def add(self, state, action, reward, next_state, done, log_prob, value, state_str):
        self.trajectories.append(Trajectory(state, action, reward, next_state, done, log_prob, value, state_str))
    
    def clear(self): self.trajectories = []
    def __len__(self): return len(self.trajectories)
    
    def compute_gae_and_normalize(self, last_value):
        """计算GAE并在内部进行奖励归一化"""
        n = len(self.trajectories)
        rewards = np.array([t.reward for t in self.trajectories])
        
        # 1. 奖励归一化
        # 使用当前batch的统计量进行归一化，这是一种'online'风格的近似，或者使用运行统计量
        # 这里为了稳定性，使用batch均值和方差
        ...
        for t in reversed(range(n)):
            if t == n - 1:
                ...
                next_non_terminal = 1.0 - dones_t[t] # 如果是最后一步且done，则为0
                # 但通常last_value是在done=False时才有效的bootstrapping
                # 如果done=True, last_value应为0，逻辑需严谨
...
class PPOAgent:
    ...
    def update(self, last_value):
        ...
        # 1. 计算GAE与归一化奖励
        advantages, returns = self.buffer.compute_gae_and_normalize(last_value)
...
def train_ppo():
    ...
    for episode in pbar:
        ...
        for step in range(config.max_steps_per_episode):
            ...
            # 计算RND内在奖励并更新探索统计
            intrinsic_reward = agent.rnd.get_intrinsic_reward(state)
            agent.rnd.update_predictor(state)
            agent.rnd.update_exploration_stats(state, intrinsic_reward)
            
            # 存入Buffer (不立即归一化)
            agent.buffer.add(state, action, reward, next_state, done, log_prob, value, array2str(state))
