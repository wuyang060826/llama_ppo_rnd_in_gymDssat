...
# 参数配置 - 基于Jupyter Notebook优化
BUFFER_SIZE = int(1e5)  # 保持与原始设置一致
BATCH_SIZE = 512        # 增大batch size以提高GPU利用率
GAMMA = 0.99            # 折扣因子
TAU = 8                 # 目标网络更新频率
LR = 1e-5               # 降低学习率以获得更稳定的训练
UPDATE_EVERY = 16       # 更频繁地更新网络
N_EPISODES = 2000       # 增加episode数量以获得更好的收敛
TOKEN_SIZE = 27
STATE_SIZE = 25
ACTION_SIZE = 25
...
CONVERGENCE_THRESHOLD = 0.1  # 收敛判断阈值（方差占比）
...
class RNDModule:
    ...
    def __init__(self, state_size, hidden_size=RND_HIDDEN_SIZE):
        ...
        self.exploration_bonus_threshold = 0.5  # 探索奖励衰减阈值
...
class MetricsTracker:
    ...
    def __init__(self):
        # 农学指标
        self.yield_list = []           # 最终产量 (kg/ha)
        self.irrigation_list = []      # 灌溉量 (mm)
        self.fertilizer_list = []      # 施肥量 (kg/ha)
        self.wue_list = []             # 水分利用率 (kg/mm)
        self.nue_list = []             # 氮肥利用率 (kg/kg)
        
        # AI指标
        self.scores = []               # 平均回报
        self.episode_steps = []        # 每轮步数
        self.total_steps = 0           # 总交互步数
        self.sample_efficiency_step = None  # 样本效率
        self.convergence_step = None   # 收敛速度
        
        # 探索相关
        self.exploration_coverage_list = []  # 探索覆盖率
...
class Agent:
    def __init__(self, state_size, action_size):
        ...
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR, betas=betas, weight_decay=weight_decay)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        # 更新探索覆盖率
        exploration_error = self.rnd.update_exploration_coverage(state)
    ...
    def save(self, name):
        """保存模型"""
        os.makedirs('/home/gymusr/bert_dqn_checkpoints', exist_ok=True)
        torch.save(self.qnetwork_local.state_dict(), f'/home/gymusr/bert_dqn_checkpoints/model{name}.pth')
        print(f'Model saved as /home/gymusr/bert_dqn_checkpoints/model{name}.pth')
        return


# ==================== 结果保存函数 ====================
def save_results(metrics_tracker, output_dir='/home/gymusr/bert_dqn_results'):
    """保存所有结果到文件"""
...
def dqn_train():
    ...
    env_args = {
        ...
        'log_saving_path': '/home/gymusr/bert_dqn_results/logs/dssat-pdi.log',
        'mode': 'all',
        'seed': 123456,
        'random_weather': True,  # 使用随机天气以增加训练多样性
    }
    ...
    eps = 1.0
    eps_decay = 0.994  # 调整epsilon衰减率
    eps_end = 0.01     # 设置epsilon最小值
    ...
    # 保存结果
    print("\nSaving results...")
    excel_path, summary_path = save_results(metrics_tracker, output_dir='/home/gymusr/bert_dqn_results')
